...
export default function HyperspaceBG({ paused = false }) {
    ...
    useEffect(() => {
        ...
        const drawFrame = (timestamp) => {
            ...
            const speed = clamp(motionPrefs.baseSpeed + state.centerX * 0, motionPrefs.baseSpeed, motionPrefs.maxSpeed);
            ...
            for (let i = 0; i < state.stars.length; i++) {
                ...
                const alpha = clamp(1 - star.z / motionPrefs.depth, 0, 1);
                const lineWidth = clamp(cur.scale * star.size * 2.2, 0.35, 3.6);
                const useAccent = star.tint < 0.32;
                ...
                ctx.beginPath();
                ctx.arc(cur.px, cur.py, lineWidth * 0.7, 0, Math.PI * 2);
