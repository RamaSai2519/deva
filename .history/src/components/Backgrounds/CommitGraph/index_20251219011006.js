...
export default function CommitGraphBG({ paused = false }) {
    ...
    useEffect(() => {
        ...
        const colors = {
            ...
            l1: getProbeColor(level1ProbeRef.current, "rgb(34, 197, 94)"),
            l2: getProbeColor(level2ProbeRef.current, "rgb(74, 222, 128)"),
            l3: getProbeColor(level3ProbeRef.current, "rgb(16, 185, 129)"),
            l4: getProbeColor(level4ProbeRef.current, "rgb(132, 204, 22)"),
        };
        ...
        const resize = () => {
            ...
            // scale cell size with viewport, but keep it looking like GitHub.
            const base = clamp(Math.min(width, height) * 0.018, 9, 15);
            state.cell = Math.floor(base);
            state.gap = clamp(Math.floor(base * 0.35), 3, 6);
            ...
            // Position like the screenshot: left-centered and slightly above middle.
            state.leftPad = clamp(width * 0.08, 24, 120);
            state.topPad = clamp(height * 0.22, 86, 220);
        };
        ...
        const drawHeatmap = () => {
            ...
            for (let c = 0; c < cols; c++) {
                for (let r = 0; r < rows; r++) {
                    ...
                    const alpha = level === 0 ? 0.25 : clamp(0.45 + shimmer * 0.35, 0.45, 0.9);
            ...
            const dotCols = 15;
            const dotRows = rows;
            const dotStartX = state.leftPad + state.gridW + clamp(state.width * 0.06, 24, 70);
            const dotCell = Math.max(10, Math.floor(state.cell * 0.92));
            const dotGap = Math.max(6, Math.floor(state.gap * 1.25));
            ...
            for (let c = 0; c < dotCols; c++) {
                for (let r = 0; r < dotRows; r++) {
                    ...
                    ctx.beginPath();
                    ctx.arc(x, y, dotCell * 0.45, 0, Math.PI * 2);
    ...
    return (
        <div className="pointer-events-none absolute inset-0 z-0" aria-hidden="true">
            ...
            <span ref={emptyProbeRef} className="hidden text-slate-700" />
            <span ref={level1ProbeRef} className="hidden text-emerald-700" />
            <span ref={level2ProbeRef} className="hidden text-emerald-600" />
            <span ref={level3ProbeRef} className="hidden text-emerald-500" />
            <span ref={level4ProbeRef} className="hidden text-lime-400" />
