...
export default function CommitGraphBG({ paused = false, word }) {
    ...
    useEffect(() => {
        ...
        const resize = () => {
            ...
            state.dotW = state.dotCols * state.dotCell + (state.dotCols - 1) * state.dotGap;

            const spacer = clamp(width * 0.06, 24, 70);
            ...
            state.topPad = clamp((height - state.gridH) / 2 - height * 0.06, 64, height * 0.32);
